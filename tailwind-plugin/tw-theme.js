...
const fontSizeConfig = themeConfig.fonts.font_size;

const calculateFontSizes = (fontSizeObj) => {
  const base = Number(fontSizeObj.base);
  const scale = Number(fontSizeObj.scale);
  const sizes = {};
  ...
  // Add any additional custom sizes from the object
  Object.entries(fontSizeObj).forEach(([key, value]) => {
    if (key !== "base" && key !== "scale") {
      sizes[key] = typeof value === "number" ? `${value}px` : value;
    }
  });

  return sizes;
};
const fontSizes = calculateFontSizes(fontSizeConfig);
